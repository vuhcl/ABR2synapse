...
from utils.feature_extraction import (
    amplitude_vs_spl_slopes,
    calculate_curvature,
    calculate_distance,
    calculate_p1_latency_ms,
    calculate_variance,
...
def load_data(verbose=False, db_level=70):
            ...
            if fq.startswith("ABR") and fq.endswith(".tsv"):
                ...
                for freq in freqs:
                    for lvl in levels:
                        ...
                        dist = calculate_distance(waveform, pks, trs)
                        amp_df_full.at[idx, "Distance"] = dist
                        amp_df_full.at[idx, "Slope"] = slope_from_amplitude_distance(
                            amp, dist
                        )
                        ...
                        amp_df_full.at[idx, "P1_Latency_ms"] = calculate_p1_latency_ms(
                            np.asarray(waveform), int(pks[0])
                        )
                        idx += 1

    _io_rows = []
    for (subj, fk), g in amp_df_full.groupby(["Subject", "Frequency(kHz)"]):
        g = g.sort_values("Level(dB)")
        sa, sh = amplitude_vs_spl_slopes(g["Level(dB)"].values, g["Amplitude"].values)
        _io_rows.append(
            {
                "Subject": subj,
                "Frequency(kHz)": fk,
    ...
    amp_df_full = amp_df_full.merge(
        pd.DataFrame(_io_rows), on=["Subject", "Frequency(kHz)"], how="left"
    )
            ...
            if fq.startswith("ABR") and fq.endswith(".tsv"):
                ...
                for freq in freqs:
                    ...
                    for lvl in levels:
                        ...
                        distances[lvl_idx] = dist
                        slopes[lvl_idx] = slope_from_amplitude_distance(amp, dist)
